...
object Tag {
  ...
  def printAllTags(actual_directory_path:String):Unit = {
    ...
    val tag_directory = new DirectoryHandler(new File(actual_directory_path+"/.sgit/refs/tags"))
  ...
  def addTag(tag:String,actual_directory_path:String): Unit = {
    val tag_directory = new DirectoryHandler(new File(actual_directory_path+"/.sgit/refs/tags"))
    ...
    if(existTag(tag,tag_directory.getFilesName)) println(Console.RED+"ERROR : tag " + tag + " already exists!")
      ...
      val actual_branch = Branch.getActualBranch(actual_directory_path)
      val last_commit = Commit.getLastCommitFromBranch(actual_branch,actual_directory_path)

      //create tag file with last commit in content
      val tag_file = new FileHandler(new File(actual_directory_path+"/.sgit/refs/tags/"+tag))
      tag_file.createFile()
