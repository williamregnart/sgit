...
class FileHandlerTest extends FunSpec with Matchers with BeforeAndAfter {
  ...
  val test_file_handler = new FileHandler(new File(test_directory.getPath+"/test_file"))
  ...
  describe("a file handler") {
    ...
    it("should in order, in File, create it, write content, write at the end, overwrite, ") {
      ...
      test_file_handler.addContent("new\n\ncontent", appendContent = false)
      test_file_handler.getContent shouldBe "new\n\ncontent\n"
    }

    it("should give the path from directory repository"){
      test_file_handler.getPathFromDir(test_directory.getPath) shouldBe "/test_file"
    }
