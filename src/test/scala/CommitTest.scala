...
class CommitTest extends FunSpec with Matchers with BeforeAndAfter{
  ...
  val branch_master_file = FileHandler(new File(test_directory.getPath+"/.sgit/refs/heads/master"))
  ...
  */
  ...
  describe("branch file"){
    it("should have the last commit in content"){
      Commit.commit(test_directory)
      branch_master_file.getContent shouldBe Commit.getLastCommitFromBranch("master",test_directory)+"\n"
    }
  }
  ...
  describe("the commit file"){
    it("should have the right content"){
      Commit.commit(test_directory)
      val commit_name = Commit.getLastCommitFromBranch("master",test_directory)
      val commit_file = FileHandler(new File(commits_directory.getPath+"/"+commit_name))
      val commit_content = commit_file.getContent

      val expected_commit_content = "tree "+index_file.getTree("",test_directory)+"\nparentTree None\ndate "+Commit.getDateToday+"\n"
