...
object Commit{
  ...
  def createCommitFile(tree:String,parent_tree:Option[String],date:String,actual_directory:File):Unit={
    ...
    val commit_File = new FileHandler(new File(actual_directory.getPath+"/.sgit/objects/commits/"+Encryption.sha1(content)))
  ...
  def addCommitToBranch(branch:String,commit:String,actual_directory:File):Unit={
    val branch_File = new FileHandler(new File(actual_directory.getPath+"/.sgit/refs/heads/" + branch))
